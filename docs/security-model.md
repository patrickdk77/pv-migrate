...
- The database password for `--flush` is read by the CLI, from a Secret in the database's namespace or from an environment variable, and written to the client's stdin over the exec connection. It never goes into a pod spec or a Secret the tool creates. The MySQL, MariaDB and PostgreSQL clients get it through their password environment variable. MongoDB's shells have none, so for `mongodb` the password is an argument to the shell inside the database's own container, visible to anyone who can list that container's processes.
...
| Credentials leak through the process list or the logs | Bucket credentials travel in a Secret and reach rclone through the environment. They can be given to the CLI through environment variables too, see [Credentials](backup-restore.md#credentials). The `--flush` password reaches the database client on its stdin, errors never quote it, and one containing a line break is refused so no part of it can run as a statement. |
