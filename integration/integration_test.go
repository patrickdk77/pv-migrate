...
func testDetachMode(t *testing.T, te *testEnv) {
	...
	job, err := k8s.FindDataMoverJob(ctx, te.sourceCli.KubeClient, te.sourceNS, releasePrefix, slogt.New(t))
	require.NoError(t, err)
	...
	require.NoError(t, runCliAppWithArgs(ctx, t, "status", "-n", te.sourceNS, "--follow", migrationID))
