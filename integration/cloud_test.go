...
func TestCloudS3(t *testing.T) {
	...
	if accessKey == "" || secretKey == "" {
		skipOrFail(t, "CLOUD_TEST_S3_ACCESS_KEY / CLOUD_TEST_S3_SECRET_KEY not set", "CLOUD_TEST_S3_MUST_RUN")
	}
...
func TestCloudGCS(t *testing.T) {
	...
	if saJSON == "" {
		skipOrFail(t, "CLOUD_TEST_GCS_SERVICE_ACCOUNT_JSON not set", "CLOUD_TEST_GCS_MUST_RUN")
	}
...
func TestCloudAzure(t *testing.T) {
	...
	if storageKey == "" {
		skipOrFail(t, "CLOUD_TEST_AZURE_STORAGE_KEY not set", "CLOUD_TEST_AZURE_MUST_RUN")
	}
