...
func BuildMigrateCmd(ctx context.Context, version, commit, date string, logger *slog.Logger) (*cobra.Command, error) {
	...
	backupCmd, err := buildBackupCmd(&logger, migration.ImageTag, migration.ChartVersion) //nolint:contextcheck
	if err != nil {
		return nil, fmt.Errorf("failed to build backup command: %w", err)
	}

	restoreCmd, err := buildRestoreCmd(&logger, migration.ImageTag, migration.ChartVersion) //nolint:contextcheck
	if err != nil {
